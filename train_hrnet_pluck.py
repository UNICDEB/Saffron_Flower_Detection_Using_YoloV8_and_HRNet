...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        print(f"\n=== Epoch {epoch+1}/{args.epochs} ===")
        train_loss, train_mae = train_one_epoch(model, train_loader, optimizer, device, criterion, scaler)
        val_loss, val_mae, val_px_dist = validate(model, val_loader, device, criterion, args.img_size)
        scheduler.step()
        ...
        if is_best:
            ...
            save_checkpoint({"model": model.state_dict(),
                             ...
                             "epoch": epoch,
                             "best_val": best_val_loss},
                             args.out.replace(".pth","_best.pth"))
